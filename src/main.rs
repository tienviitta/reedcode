...
fn main() {
    ...
    println!(
        "  #2b: Add Two Numbers: {:?}",
        add_two_num_std::add_two_num_std(lil0, lil1)
    );

    let s = String::from("abcabcbb");
    let len = length_of_longest_substring::length_of_longest_substring(s);
    println!(
        "  #3a: Longest Substring Without Repeating Characters: {}",
        len
    );
    let s = String::from("abcabcbb");
    let len = length_of_longest_substring::length_of_longest_substring_lc(s);
    println!(
        "  #3b: Longest Substring Without Repeating Characters: {}",
        len
    );
}
